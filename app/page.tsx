...
// その日が属する週の月曜日（YYYY-MM-DD）。DB側 date_trunc('week') と揃える。
...
// 自分の記録を週ごとにまとめる（同じ週に複数回記録したら平均）。
type WeekAgg = { key: string; label: string; overall: number; items: number[] };
function buildWeekly(sorted: MyResponse[]): WeekAgg[] {
  const map = new Map<string, MyResponse[]>();
...
// 返り値: weeks=週ラベルの配列, series=項目ごとの{key, values} 配列（valuesは週順のばらつき）。
...
export default function App() {
  ...
  const [itemTimeline, setItemTimeline] = useState<Spread[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [r, is, tl, itl] = await Promise.all([
          getMyResponses(),
          getTeamItemSpread(teamId),
          getTeamSpreadTimeline(teamId),
          getTeamItemSpreadTimeline(teamId),
        ]);
        setResponses(r);
        setItemSpread(is);
        setTimeline(tl);
  ...
  const refreshShare = async () => {
    if (!teamId) return;
    try {
      const [is, tl, itl] = await Promise.all([
        getTeamItemSpread(teamId),
        getTeamSpreadTimeline(teamId),
        getTeamItemSpreadTimeline(teamId),
      ]);
    ...
    <div className="root">
      ...
      <main>
        {view === "record" ? <RecordView draft={draft} setDraft={setDraft} memo={memo} setMemo={setMemo} onSave={handleSave} saving={saving} chkTranslate={chkTranslate} setChkTranslate={setChkTranslate} chkDevice={chkDevice} setChkDevice={setChkDevice} />
          ...
          : <ShareView slot={teamSlot} itemSpread={itemSpread} timeline={timeline} itemTimeline={itemTimeline} />}
      </main>
...
// 7項目を1枚にまとめた折れ線グラフ。
// centered=true: 中央0基準（個人の項目別平均、-50..50）／ false: 下端0基準（ばらつき）。
            ...
            <>
              {itemPivot.weeks.length > 0 ? (
                ...
                <>
                  ...
                  <p className="note">※項目ごとの推移を出すには、管理者がSupabaseで <code>team_item_spread_timeline</code> 関数を追加する必要があります。</p>
                </>
              ) : (
                <p className="note" style={{ textAlign: "center", padding: "20px 0" }}>
